...
exports.postEmailconfirmation = (req, res) => {
    ...
    Token.findOne({ token: req.params.token }, (error, token) => {
        if (error || !token) {
            return res.status(400).json({ error: "invalid tokrn or token not found" })
        }
        ...
        User.findOne({ _id: token.userId }, (error, user) => {
            ...
            if (user.isVerified) {
                return res.status(400).json({ error: "email is already verified please login to confirm" })
            }
            ...
            user.save((error) => {
                ...
                res.json({ message: "congratulation, your account has been verified,login to continue" })
            })
        })
